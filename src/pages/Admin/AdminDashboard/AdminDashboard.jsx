...
const AdminDashboard = () => {
  ...
  const [tempData, setTempData] = React.useState({ ...fieldData });

  const fieldGroup1 = [
    { id: 'full-name', label: 'Full name', placeholder: 'Jimmy Smith', icon: 'user' },
    { id: 'phone-number', label: 'Phone number', placeholder: '+12345678910', icon: 'call' },
    { id: 'address', label: 'Address', placeholder: 'Address', icon: 'home' },
  ];

  const fieldGroup2 = [
    { id: 'email', label: 'E-mail Address', placeholder: 'Email', icon: 'direct' },
    { id: 'password', label: 'Password', placeholder: 'Password', icon: 'key' },
    { id: 'postal-code', label: 'Postal code', placeholder: 'Postal code', icon: 'signpost' },
  ];
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      case 'signpost':
        return (
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <path d="M15 22.75H9C8.59 22.75 8.25 22.41 8.25 22C8.25 21.59 8.59 21.25 9 21.25H15C15.41 21.25 15.75 21.59 15.75 22C15.75 22.41 15.41 22.75 15 22.75Z" fill="currentColor" />
          </svg>
        );
      case 'edit':
        return (
      ...
      case 'save':
        return (
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
            <path d="M12.92 22.75H6.42C3.12 22.75 1.25 20.88 1.25 17.58V6.42C1.25 3.12 3.12 1.25 6.42 1.25H12.91C16.21 1.25 18.08 3.12 18.08 6.42V17.58C18.08 20.88 16.22 22.75 12.92 22.75ZM6.42 2.75C4.01 2.75 2.75 4.01 2.75 6.42V17.58C2.75 19.99 4.01 21.25 6.42 21.25H12.91C15.32 21.25 16.58 19.99 16.58 17.58V6.42C16.58 4.01 15.32 2.75 12.91 2.75H6.42Z" fill="currentColor" />
            <path d="M21.5799 15.2501H18.0799C17.6699 15.2501 17.3299 14.9101 17.3299 14.5001C17.3299 14.0901 17.6699 13.7501 18.0799 13.7501H21.5799C21.9899 13.7501 22.3299 14.0901 22.3299 14.5001C22.3299 14.9101 21.9899 15.2501 21.5799 15.2501Z" fill="currentColor" />
            <path d="M22.0002 18.2501H18.0802C17.6702 18.2501 17.3302 17.9101 17.3302 17.5001C17.3302 17.0901 17.6702 16.7501 18.0802 16.7501H22.0002C22.4102 16.7501 22.7502 17.0901 22.7502 17.5001C22.7502 17.9101 22.4102 18.2501 22.0002 18.2501Z" fill="currentColor" />
            <path d="M21.1301 12.2501H18.0801C17.6701 12.2501 17.3301 11.9101 17.3301 11.5001C17.3301 11.0901 17.6701 10.7501 18.0801 10.7501H21.1301C21.5401 10.7501 21.8801 11.0901 21.8801 11.5001C21.8801 11.9101 21.5401 12.2501 21.1301 12.2501Z" fill="currentColor" />
  ...
  const handleFieldChange = (id, value) => {
    setTempData(prev => ({ ...prev, [id]: value }));
  };

  const clearField = (id) => {
    setTempData(prev => ({ ...prev, [id]: '' }));
  };

  const handleEdit = (id) => {
    setTempData({ ...fieldData });
    setEditingField(id);
  };

  const handleSave = (id) => {
    setFieldData(prev => ({ ...prev, [id]: tempData[id] }));
  ...
  const renderField = (field) => {
    ...
    const value = isEditing ? tempData[field.id] : fieldData[field.id];
    ...
    return (
      <div
        key={field.id}
        className={`account-input-group ${focusedField === field.id ? 'focused' : ''} ${isError ? 'error' : ''} ${isFilled ? 'filled' : ''} ${isEditing ? 'editing' : ''}`}
      >
        <div className="label-container">
          <span className="label-segment segment-left"></span>
          <label className="input-label" htmlFor={`input-${field.id}`}>{field.label}</label>
        ...
        <div className="account-input-container">
          ...
          <div className="input-left">
            <span className="input-icon">{getIcon(field.icon)}</span>
            {isEditing ? (
              <input
                id={`input-${field.id}`}
              ...
              <span className="field-display-text">{value || field.placeholder}</span>
            )}
          </div>
          <div className="input-right">
            ...
            <button
              className="edit-btn"
              onClick={() => isEditing ? handleSave(field.id) : handleEdit(field.id)}
              aria-label={isEditing ? `Save ${field.label}` : `Edit ${field.label}`}
            >
              {isEditing ? getIcon('save') : getIcon('edit')}
            </button>
          </div>
        </div>
      </div>
